...
def test_idempotency_same_key_returns_same_transaction_id():
    ...
    body = {
        "type": "payin",
        "merchant_data": {**MERCHANT_DATA, "order_id": gen_order_id("idem_qr")},
        "financial_data": {"amount": 10000, "currency": "RUB"},
        "transaction_data": {"method": "qr"},
        "flow_data": {"is_recurrent": False, "capture_mode": "auto", "threed_secure": THREED},
        "customer_data": CUSTOMER_DATA,
...
@pytest.mark.tcid("QR-19-1")
def test_qr_amount_with_kopecks():
    """Оплата QR/СБП — сумма с копейками (10050 = 100.50 руб). Ожидается 201 и amount=10050 в ответе."""
    body = {
        **_QR_BASE,
