...
def test_gate_return_data_also_uses_new_result_url():
    ...
    path  = f"{_BASE_PATH}/{token}/gate/return/data"
    resp  = requests.post(
        f"{_WEB3_HOST}{path}",
        data="PaRes=eJxVUstuwjAQ%2B%2B%2Bh&MD=12345",
        headers={"Content-Type": "application/x-www-form-urlencoded", "X-SPG-Origin": _SPG_ORIGIN},
        allow_redirects=False,
        timeout=_cfg.HTTP_TIMEOUT,
    )
    assert resp.status_code == 302, f"Expected 302, got {resp.status_code}: {resp.text[:200]}"
